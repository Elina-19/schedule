...
namespace GoogleParser
{
    public class GoogleTableParser
    {
        ...
        public static void TableAccess()
        {
            ...
            using (var stream = new FileStream(@"GoogleTableParser\table_parser.json", FileMode.Open, FileAccess.Read))
            {
        ...
        private static void WriteToJson( List<Subject> subjects )
        {
            File.WriteAllText( @"result.json", JsonConvert.SerializeObject( subjects, Formatting.Indented ) );
        }
